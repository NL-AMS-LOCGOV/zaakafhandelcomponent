...
export class GoogleMapsService {
    ...
    constructor(@Inject(BUILDER_CONFIG) private config: MaterialFormBuilderConfig, private httpClient: HttpClient) {
    }
    ...
    public load(): void {
        ...
        const loading = this.httpClient.jsonp(`https://maps.googleapis.com/maps/api/js?key=${this.config.googleMapsApiKey}&libraries=places`, 'callback')
                            .pipe(
                                map(() => true),
                                catchError(() => of(false))
                            );
