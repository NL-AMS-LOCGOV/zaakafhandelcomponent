...
export class DocumentenLijstComponent extends FormComponent implements OnInit {
    ...
    columns: string[] = ['select', 'titel', 'documentType', 'status', 'versie', 'auteur', 'creatiedatum', 'bestandsomvang', 'ondertekening', 'url'];
    ...
    ngOnInit(): void {
        ...
        this.data.documenten$.subscribe(documenten => {
            for (const document of documenten) {
                ...
                if(document.ondertekening.datum && document.ondertekening.soort) {
    ...
    isOndertekend(document): boolean {
        return this.ondertekend.indexOf(document.uuid) > -1;
    }
