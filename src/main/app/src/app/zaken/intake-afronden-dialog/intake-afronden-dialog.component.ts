...
export class IntakeAfrondenDialogComponent implements OnInit, OnDestroy {
    ...
    constructor(public dialogRef: MatDialogRef<IntakeAfrondenDialogComponent>,
        ...
        this.afzenders = this.zakenService.listAfzendersVoorZaak(this.data.zaak.uuid);
        this.mailtemplateService.findMailtemplate(Mail.ZAAK_ONTVANKELIJK, this.data.zaak.uuid)
            .subscribe(mailtemplate => {
                this.zaakOntvankelijkMail = mailtemplate;
            });
        this.mailtemplateService.findMailtemplate(Mail.ZAAK_NIET_ONTVANKELIJK, this.data.zaak.uuid)
            .subscribe(mailtemplate => {
    ...
    ngOnInit(): void {
        ...
        this.zakenService.readDefaultAfzenderVoorZaak(this.data.zaak.uuid).subscribe(afzender => {
        ...
        this.formGroup.get('sendMail').valueChanges.pipe(takeUntil(this.ngDestroy)).subscribe(value => {
            this.formGroup.get('verzender').setValidators(value ? [Validators.required] : null);
