...
export class BesluitViewComponent implements OnInit, OnChanges {
    ...
    private maakIdField(besluit: Besluit): HiddenFormField {
        return new HiddenFormFieldBuilder(besluit.uuid)
        .id('uuid')
        .build();
    }
    ...
    private maakVervalredenField(besluit: Besluit): SelectFormField {
        const vervalRedenen = this.utilService.getEnumAsSelectListExceptFor('besluit.vervalreden',
            VervalReden, [VervalReden.TIJDELIJK]);
        const vervalReden = besluit.vervalreden ? {
            label: this.translate.instant('besluit.vervalreden.' + besluit.vervalreden),
    ...
    private maakMessageField(besluit: Besluit): MessageFormField {
        const documentenVerstuurd: boolean = besluit.informatieobjecten
                                                    .some(document => {
